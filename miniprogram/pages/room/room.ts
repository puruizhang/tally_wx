...
Page({
  ...
  init(action :any) {
    if (this.data.roomId) {
      if('socket' == action){
        post('roomCommonInfo', { roomId: this.data.roomId },false).then((res: any) => {
          this.setData({
            ...res
          })
        })
  ...
  submitMoney() {
    post('roomTransfer', {
      ...
      this.init();
      Toast('转账成功');
      console.log('发送socket转账给用户' + this.data.payeeUserId)
      wx.sendSocketMessage({
        data: '转账给用户' + this.data.payeeUserId
      })
      console.log('发送socket转账给用户完成')
    })
  },
  toIndex() {
    Dialog.confirm({
